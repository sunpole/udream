(function(){
    ...
    // флаг, показывать ли инструкцию (изначально показываем)
    ...
    function setLang(l) {
        ...
        applyLocalization();
        window.scrollTo(0, scrollY); // сохраняем позицию
    }
    ...
    function filterByMode(item, query) {
        ...
        if(currentMode === "symbol") {
            return item.symbol.toLowerCase().startsWith(q) || (item.aliases && item.aliases.some(a => a.toLowerCase().startsWith(q)));
        } else if(currentMode === "aliases") {
            return item.aliases && item.aliases.some(a => a.toLowerCase().startsWith(q));
        } else if(currentMode === "desc") {
            return (item.description || "").toLowerCase().startsWith(q);
        } else if(currentMode === "tags") {
            return item.tags && item.tags.some(t => t.toLowerCase().startsWith(q));
        } else if(currentMode === "all") {
            return item.symbol.toLowerCase().startsWith(q) ||
                   (item.aliases && item.aliases.some(a => a.toLowerCase().startsWith(q))) ||
                   (item.description || "").toLowerCase().startsWith(q) ||
                   (item.tags && item.tags.some(t => t.toLowerCase().startsWith(q)));
        }
        return false;
    }
    ...
    function updateAutocomplete() {
        ...
        if(matches.length) {
            ...
            // даём время на отрисовку, чтобы получить реальную высоту
    ...
    function performSearch() {
        ...
        if(!q) {
            ...
            else resultCard.innerHTML = `<div>${t("enterQuery")}</div>`;
            return;
        }
        let filtered = db.filter(item => {
            if(currentMode === "symbol") {
                return item.symbol.toLowerCase().includes(q) || (item.aliases && item.aliases.some(a => a.toLowerCase().includes(q)));
            } else if(currentMode === "aliases") {
                return item.aliases && item.aliases.some(a => a.toLowerCase().includes(q));
            } else if(currentMode === "desc") {
                return (item.description || "").toLowerCase().includes(q);
            } else if(currentMode === "tags") {
                return item.tags && item.tags.some(t => t.toLowerCase().includes(q));
            } else {
    ...
    function showDefaultInstructions() {
        ...
        document.getElementById("closeInstructionBtn").addEventListener("click", () => {
            instructionVisible = false;
            resultCard.innerHTML = `<div>${t("enterQuery")}</div>`;
        });
    }
            ...
            inp.onchange = e => {
                ...
                reader.onload = function(e) {
                    try {
                        ...
                        if(Array.isArray(data) && data.length) {
                            db = data; currentDbName = file.name; updateStatsUI(); renderAlphabet();
                            updateTagCloudVisibility(); showDefaultInstructions(); dbLoaded = true;
