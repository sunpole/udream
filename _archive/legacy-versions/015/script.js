(function(){
    ...
    // настройки видимости (по умолчанию всё включено, кроме хлебных крошек и тегов – они выключены)
    ...
    function applyLocalization() {
        ...
        updateStatsUI();
        updateDbInfoPanel();
        const thanksDiv = document.querySelector(".footer div:first-child");
    ...
    function setLang(l) { lang = l; localStorage.setItem("clientLang", l); updateLangToggleButton(); applyLocalization(); if (lastDisplayedRecord) showRecord(lastDisplayedRecord); else showDefaultInstructions(); }
    ...
    function updateBreadcrumbsVisibility() {
        if(showBreadcrumbs) {
            breadcrumbsArea.style.display = "flex";
            historyNav.style.display = "flex";
        } else {
            breadcrumbsArea.style.display = "none";
    ...
    async function shareAsImage(record) {
        ...
        try {
            ...
            if(blob.size > 500*1024) {
                // уменьшаем качество
                const smallerBlob = await new Promise(resolve => canvas.toBlob(resolve, 'image/jpeg', 0.5));
                if(smallerBlob.size > 500*1024) {
                    alert("Изображение всё ещё большое, но попробуем поделиться");
                }
                if(navigator.share) {
                    const file = new File([smallerBlob], `${record.symbol}.jpg`, { type: 'image/jpeg' });
