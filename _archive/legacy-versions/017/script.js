(function(){
    ...
    let historyStack = [];          // для хлебных крошек (старый)
    let historyIndex = -1;
    let lastDisplayedRecord = null;
    let dbLoaded = false;
    let currentDbName = "db.json";

    // расширенная история (с timestamp)
    ...
    // сортировка тегов: 'alpha' или 'count'
    ...
    function updateStatsUI() {
        if(!db.length) { statsPanel.innerHTML = "База не загружена"; return; }
    ...
    function renderFullHistory() {
        ...
        if(!fullHistory.length) {
            historyBlock.innerHTML = '<div>История пуста</div>';
            return;
        }
    ...
    function renderAlphabet() {
        ...
        document.querySelectorAll(".alpha-letter[data-letter]").forEach(el => {
            el.addEventListener("click", () => {
                ...
                if(filtered.length) showWordList(filtered, `${lang==='ru'?'Символы на букву':'Symbols starting with'} ${letter} (${filtered.length})`);
    ...
    function updateAutocomplete() {
        ...
        const matches = db.filter(item => item.symbol.toLowerCase().includes(q) || (item.aliases && item.aliases.some(a=>a.toLowerCase().includes(q)))).slice(0,7);
        if(matches.length) {
            autocompleteList.innerHTML = matches.map(m => `<div class="autocomplete-item" data-id="${m.id}">${escapeHtml(m.symbol)}</div>`).join("");
            autocompleteList.classList.add("show");
            autocompleteSpacer.style.height = autocompleteList.scrollHeight + "px";
    ...
    function showDefaultInstructions() {
        resultCard.innerHTML = `<div style="padding:1rem"><h3>📖 ${lang==='ru'?'Как пользоваться словарём':'How to use the dictionary'}</h3><p style="margin:0.5rem 0; font-size:0.85rem;">${lang==='ru'?'Откройте меню (гамбургер) — там можно включить дополнительные элементы навигации.':'Open menu (hamburger) to enable extra navigation elements.'}</p><p><strong>Curtains (занавесы):</strong> (1) Плотская завеса; (2) Сердце; (3) Небеса; (4) Покрытие; (5) Окончание; (6) Смерть.<br><em>См. также Veil.</em><br>(1) Евр.10:20; (2) 2Кор.3:15; (3) Пс.103:2; Ис.40:22</p><ul><li>• Нумерованные толкования от наиболее вероятного к наименее.</li><li>• Скобки уточняют контекст.</li><li>• Перекрёстные ссылки.</li></ul><p><strong>Life Raft:</strong> (1) Нужда в спасении; (2) Потерянный; (3) В опасности потерять спасение; (4) Ковчег (Христос).<br><em>См. также Adrift, Boat, Sea.</em></p><p>• cf. = сравни, ff. = и следующие.</p><p><strong>Mouse/Mice:</strong> (1) Скрытый нечистый дух; (2) Показатель духовного обслуживания; (3) Неверующий (нечистый); (4) Малый; (5) Язва; (6) Суд.<br>‘&’ = объединить стихи.</p></div>`;
    }
    ...
    function updateSwitchesUI() {
        ...
        if(showHistoryBlock) toggleHistoryBlockCont.classList.add("active"); else toggleHistoryBlockCont.classList.remove("active");
        if(allowSelection) toggleSelectionCont.classList.add("active"); else toggleSelectionCont.classList.remove("active");
        if(wideScrollbar) toggleScrollbarCont.classList.add("active"); else toggleScrollbarCont.classList.remove("active");
    }
            ...
            try {
                ...
                if(resp.ok) {
                    ...
                    if(Array.isArray(data) && data.length) {
                        ...
                        updateStatsUI(); renderAlphabet(); updateTagCloudVisibility(); showDefaultInstructions(); dbLoaded = true;
                        return;
                    }
                }
            } catch(e) {}
    ...
    function init() {
        ...
        clearHistoryBtn.onclick = () => {
            if(confirm(lang==='ru'?'Удалить всю историю?':'Delete all history?')) {
                clearAllHistory();
