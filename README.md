...
Проект работает непосредственно в браузере: сервер, регистрация и база данных на стороне сервера не требуются. Текущий интерфейс использует единую версию `UDREAM v23.7.0`. Метка `v19` сохраняется только как историческая интерфейсная итерация в архиве.
...
| Параметр | Значение |
|---|---|
| Стабильная точка восстановления | `v23.7.0` |
| Текущая версия приложения | `v23.7.0` |
| Текущая линия разработки | после `v23.7.0` функциональная разработка ещё не начата |
