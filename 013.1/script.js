// script.js — UDREAM v13.1 (исправленный)
(function() {
    // --- состояние
    let db = [];
    ...
    // i18n (как в предыдущей версии, без изменений)
    const i18n = {
        ru: {
    ...
    function updateDbInfoPanel() {
        ...
        let sizeStr = (totalChars / 1024).toFixed(1) + " KB";
        if(totalChars > 1024*1024) sizeStr = (totalChars / (1024*1024)).toFixed(1) + " MB";
        dbInfoPanel.innerHTML = t("dbInfo", { name: currentDbName || "db.json", count: db.length, chars: totalChars, size: sizeStr });
    }
    ...
    // Автоматическая загрузка из /udream/data/db.json
    async function loadMainDatabase() {
        resultCard.innerHTML = `<div class="loader"></div><div style="text-align:center">${t("loading")}</div>`;
        // Пытаемся загрузить из корня сайта /udream/data/db.json
        const url = "/udream/data/db.json";
        try {
            ...
            if (Array.isArray(data) && data.length) {
                db = data;
                currentDbName = "db.json (основная)";
                updateStatsUI(); updateDbInfoPanel();
                buildAlphabetRows();
                rebuildTagsCloud();
                showRecord(db[0]);
            ...
            console.warn("Не удалось загрузить основную базу", e);
        }
        // Если не загрузилась, пробуем альтернативные пути (на случай если папка другая)
        const altPaths = [
            "/data/db.json",
            "./data/db.json",
            "../data/db.json",
            window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/')+1) + "data/db.json"
        ];
        for (let alt of altPaths) {
            try {
        ...
        // Если ничего не загрузилось – показываем сообщение и кнопку ручной загрузки
        db = [];
        updateStatsUI(); updateDbInfoPanel();
    ...
    // Логотип – переход на главную страницу проекта (корень, а не первую запись)
