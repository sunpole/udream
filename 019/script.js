(function(){
    ...
    let showBreadcrumbs = localStorage.getItem("showBreadcrumbs") !== "false";
    ...
    const i18n = {
        ru: {
            ...
            thanks: "Благодарность Супруге — за вдохновение. Я часть Церкви «Семья Божья», г. Минск. Живите с миром.",
        ...
        en: {
            ...
            thanks: "Thanks to my wife for inspiration. I'm part of the Family of God Church, Minsk. Live in peace.",
    ...
    async function shareAsImage(record) {
        ...
        tempDiv.innerHTML = `<h1 style="font-size:24px;margin-bottom:10px">${escapeHtml(record.symbol)}</h1><p style="font-size:14px;color:#666">${record.source?'Источник: '+escapeHtml(record.source):''} ${record.date_added?'📅 '+record.date_added:''}</p><div style="font-size:16px;margin:10px 0;white-space:pre-wrap">${escapeHtml(record.description)}</div>${record.aliases?'<p><b>Синонимы:</b> '+record.aliases.join(', ')+'</p>':''}${record.tags?'<p><b>Теги:</b> '+record.tags.join(', ')+'</p>':''}${record.notes?'<div style="font-size:14px;border-top:1px solid #ccc;margin-top:10px;padding-top:10px">'+(typeof marked !== 'undefined' ? marked.parse(record.notes) : escapeHtml(record.notes))+'</div>':''}`;
    ...
    // Объявления DOM-элементов (должны быть после загрузки DOM, но они в функции init)
